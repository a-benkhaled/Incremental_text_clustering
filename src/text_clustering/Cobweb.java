...
public class Cobweb 
  ...
  private class CNode 
    ...
    /**
     * Cluster name for text
     */
    private String m_clusterName = "";
    /**
     * Cluster classe
     */
    private String m_clusterClasse = "";
    ...
    private String getClusterClasse(String separator){
    	if (m_children == null){
    		...
    		int max = 0;
    		//assigner la classe
    		for(String classe: childrenClasses.keySet()){
      		  if (max < childrenClasses.get(classe)){
      			  m_clusterClasse = classe;
	...
	throw new Exception("assignClusterNums: tree not built correctly!");
      ...
      cl_num[0]++;
      if (m_children != null) {
    	  //System.out.println(m_clusterInstances.numInstances());
	for (int i = 0; i < m_children.size(); i++) {
	  CNode child = (CNode) m_children.elementAt(i);
	...
	for (int i = 0; i < m_children.size(); i++) {
	  ...
	  text.append("node "+m_clusterNum+" ["
		      +m_clusterInstances.numInstances() + " "+ m_children.toString()
		      +"]");
    ...
    protected void graphTree(StringBuffer text) throws Exception {
      ...
      text.append("N"+m_clusterNum
		  + " [label=\""+((m_children == null) 
				  ? m_clusterInstances.toString() : "node "+m_clusterClasse)
		  +" "+m_clusterNum+" "
		  +" ("+m_clusterInstances.numInstances()
		  +")\" "
		  +((m_children == null) 
	...
	for (int i = 0; i < m_children.size(); i++) {
	  CNode temp = (CNode)m_children.elementAt(i);//fils
	  temp.graphTree(text);
	}
      }
    }
    ...
    private void getNodeClass(){
    	if(m_children == null)//feuille
    		return ;//traitement
    	for (int i = 0; i < m_children.size(); i++) {//fils
    		  CNode temp = (CNode)m_children.elementAt(i);
    		  temp.getNodeClass();
...
/**
  ...
  public void buildClusterer() throws Exception {
    ...
    getCapabilities().testWithFail(initDocSet);
  ...
  public float hierarchieMicroPrecision() {
	  ...
	  m_cobwebTree.falsePositiveSum(falsePsum);
	  
	  return ((float)truePsum[0]/(truePsum[0] + falsePsum[0]));
  }
  /**
